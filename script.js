...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function (e) {
        if (this.closest('.wizard-container')) {
            return;
        }
        e.preventDefault();
...
function updateLogo(theme) {
    if (logoImg) {
        if (theme === 'dark') {
            logoImg.src = 'https://storage.yandexcloud.net/fotora.ru/f69f7a2d021cf226.png';
        } else {
            logoImg.src = 'https://storage.yandexcloud.net/fotora.ru/93a823f923abf5a6.png';
        }
    }
}
